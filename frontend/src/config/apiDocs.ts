...
curl -X POST "{{baseUrl}}/api/v1/base/cache/clear" \\
  ...
  'debate': {
    title: 'Debate API',
    description: 'Generate AI-powered multi-agent debates with Qwen3-TTS audio output.',
    endpoint: {
      method: 'POST',
      path: '/api/v1/debate/create'
    },
    docsAnchor: '#/debate/create_debate_api_v1_debate_create_post',
    requestHeaders: [
      { name: 'Content-Type', value: 'application/json', description: 'Request body format' },
      { name: 'X-API-Key', value: 'your-api-key', description: 'API authentication key' }
    ],
    requestParams: [
      { name: 'topic', type: 'string', required: true, description: 'Debate topic or question' },
      { name: 'speakers', type: 'SpeakerConfig[]', required: true, description: 'Array of 2-6 speaker configurations' },
      { name: 'max_rounds', type: 'number', required: false, description: 'Maximum debate rounds (default: 10)' },
      { name: 'auto_advance', type: 'boolean', required: false, description: 'Auto-advance between speakers' },
      { name: 'delay_between_speakers', type: 'number', required: false, description: 'Delay in seconds between speakers' }
    ],
    requestExample: {
      topic: 'AI regulation should be strict',
      max_rounds: 10,
      auto_advance: true,
      delay_between_speakers: 1.5,
      speakers: [
        {
          id: 'speaker_0',
          name: 'Alice',
          personality: 'Progressive advocate for AI regulation',
          model_name: 'meta-llama/Llama-3.1-8B-Instruct',
          voice_description: 'Warm female voice, German',
          language: 'German',
          voice_prompt_id: ''
        },
        {
          id: 'speaker_1',
          name: 'Bob',
          personality: 'Conservative skeptic of AI regulation',
          model_name: 'meta-llama/Llama-3.1-8B-Instruct',
          voice_description: 'Calm male voice, German',
          language: 'German',
          voice_prompt_id: ''
        }
      ]
    ...
    responseHeaders: [
      { name: 'X-Session-ID', description: 'Debate session ID' }
    ],
    curlExample: `curl -X POST "{{baseUrl}}/api/v1/debate/create" \\
  -H "Content-Type: application/json" \\
  -H "X-API-Key: your-api-key" \\
  -d '{
    "topic": "AI regulation should be strict",
    ...
    "delay_between_speakers": 1.5,
    "speakers": [
      {"id": "speaker_0", "name": "Alice", "personality": "Progressive advocate", "voice_description": "Warm female voice"},
      {"id": "speaker_1", "name": "Bob", "personality": "Conservative skeptic", "voice_description": "Calm male voice"}
    ]
  }'`
  },
  'archive': {
    title: 'Archive API',
    description: 'Store and retrieve previous debates, voice prompts, and audio clips.',
    endpoint: {
      method: 'GET',
      path: '/api/v1/archive/debates'
    },
    docsAnchor: '#/archive/list_debates',
    requestHeaders: [
      { name: 'X-API-Key', value: 'your-api-key', description: 'API authentication key' }
    ],
    requestExample: null,
    ...
    responseHeaders: [],
    curlExample: `# List archived debates
curl "{{baseUrl}}/api/v1/archive/debates" -H "X-API-Key: your-api-key"

# Get specific debate
curl "{{baseUrl}}/api/v1/archive/debates/{session_id}" -H "X-API-Key: your-api-key"

# Download all as ZIP
curl "{{baseUrl}}/api/v1/archive/download-zip" -H "X-API-Key: your-api-key" -o archive.zip

# List voice prompts
curl "{{baseUrl}}/api/v1/archive/prompts" -H "X-API-Key: your-api-key"

# List audio clips
curl "{{baseUrl}}/api/v1/archive/clips" -H "X-API-Key: your-api-key"`
  }
};
