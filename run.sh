...
detect_conda_env() {
    ...
    if command -v conda &> /dev/null; then
        conda_cmd="conda"
    elif [ -x "/notebooks/workspace/bkg/miniconda/bin/conda" ]; then
        conda_cmd="/notebooks/workspace/bkg/miniconda/bin/conda"
    fi

    if [ -n "$conda_cmd" ]; then
        for env in qwen-tts qwen3-tts; do
            if $conda_cmd env list | grep -q "^$env "; then
                echo "$env"
                return
            fi
        done
    ...
    # Fallback: check by directory existence
    for env in qwen-tts qwen3-tts; do
        if [ -d "/notebooks/workspace/bkg/miniconda/envs/$env" ] || \
           [ -d "$HOME/miniconda/envs/$env" ] || \
           [ -d "$HOME/.conda/envs/$env" ]; then
            echo "$env"
            return
        fi
    done
...
activate_conda() {
    ...
    if command -v conda &> /dev/null; then
        ...
        local conda_base="/notebooks/workspace/bkg/miniconda"
    else
        echo "❌ Cannot find conda executable"
        exit 1
    fi
    ...
    # Fix CUDA library path — ensure conda env's nvidia libs are found first
    # (nvjitlink must come before cusparse to avoid version mismatch)
    local nv_base="/notebooks/workspace/bkg/miniconda/envs/$env_name/lib/python3.12/site-packages/nvidia"
    export LD_LIBRARY_PATH="${nv_base}/nvjitlink/lib:${nv_base}/cusparse/lib:${nv_base}/cublas/lib:${nv_base}/cuda_runtime/lib:${LD_LIBRARY_PATH}"
...
# ==============================================================================
# Kill stale server & bore processes
# ==============================================================================

echo "🧹 Killing stale uvicorn and bore processes ..."
...
# ==============================================================================
# Start bore tunnel (after server is confirmed ready)
# ==============================================================================

BORE_BIN="/notebooks/workspace/bkg/bin/bore"
BORE_LOG="/notebooks/workspace/bkg/logs/bore-${SERVER_PORT}.log"
if [ -x "$BORE_BIN" ]; then
    echo ""
    echo "🌐 Starting bore tunnel for port $SERVER_PORT ..."
    rm -f "$BORE_LOG"
    nohup "$BORE_BIN" local "$SERVER_PORT" --to bore.pub > "$BORE_LOG" 2>&1 &
    BORE_PID=$!
    sleep 3
    BORE_PORT=$(grep -oP 'listening at .+:\K\d+' "$BORE_LOG" 2>/dev/null | tail -1)
    if [ -n "$BORE_PORT" ]; then
        echo ""
        echo "✅ Bore tunnel active: http://bore.pub:${BORE_PORT}"
        echo "   API:  http://bore.pub:${BORE_PORT}"
        echo "   Docs: http://bore.pub:${BORE_PORT}/docs"
    else
        echo "⚠️  Bore tunnel may not be ready yet (check $BORE_LOG)"
    fi
fi
